...
myest_repfdr<-function(data,q){
	...
	y<-data[,2]
	fit1<-glm(y[which(data$C==1)]~Xc[which(data$C==1),],family="binomial")
	Z1=summary(fit1)$coef[-1,3]
	fit2<-glm(y[which(data$C==2)]~Xc[which(data$C==2),],family="binomial")
	Z2=summary(fit2)$coef[-1,3]
	Zmat=cbind(Z1,Z2)
