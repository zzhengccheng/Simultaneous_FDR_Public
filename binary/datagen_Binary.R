...
datagen<-function(n1,n2,p,s0,s1,s2,rho1,rho2,sigma1,sigma2,samesig,scale=1.2){
	...
	###sigma1 intercept parameter for population 1
	###sigma2 intercept parameter for population 2
	...
	y1<-rbinom(n1,size=1,prob=exp(sigma1+X1%*%beta1)/(1+exp(sigma1+X1%*%beta1)) )
	y2<-rbinom(n2,size=1,prob=exp(sigma2+X2%*%beta2)/(1+exp(sigma2+X2%*%beta2)) )
	data1<-cbind(1,y1,X1)
	data2<-cbind(2,y2,X2)
