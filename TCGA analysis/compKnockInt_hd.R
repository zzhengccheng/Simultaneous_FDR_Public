...
myest_int<-function(data,q,method="Max"){
	...
	y1<-data1[,2]
	y2<-data2[,2]
	X1Knock<-create.second_order(X1c,shrink=T)
	X2Knock<-create.second_order(X2c,shrink=T)
	...
	mythred1=knockoff.threshold(W1,fdr=q,offset=0)
	myselect1=which(W1>=mythred1)
	mythred2=knockoff.threshold(W2,fdr=q,offset=0)
